...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for idx in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            
            if (idx % self.num_grad_steps_per_target_update) == 0:
                values_prime = self(next_observations).squeeze()
                target_values = rewards + self.gamma * values_prime * (1-terminals)
