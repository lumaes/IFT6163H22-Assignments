...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        obs.append(ob)
        ac = policy.get_action(ob) #DONE: HINT: query the policy's get_action function
        ac = ac[0]
        acs.append(ac)
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        ...
        rollout = sample_trajectory(env,
                                    policy,
                                    max_path_length,
                                    render=render,
                                    render_mode=render_mode)
        # append the sampled trajectory to paths
        paths.append(rollout)

        # increment timesteps count based on the number of reward collected.
        timesteps_this_batch += get_pathlength(rollout)
