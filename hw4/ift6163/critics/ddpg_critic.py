...
class DDPGCritic(BaseCritic):
    ...
    def __init__(self, actor, hparams, optimizer_spec, **kwargs):
        ...
        # TD3 PARAMETERS
        self.td3_noise_clip = hparams['td3_noise_clip']
        self.td3_noise = hparams['td3_target_policy_noise']
        self.td3_two_q_net = hparams['td3_two_q_net']

        if self.td3_two_q_net:
            self.q2_net = ConcatMLP(
               hparams['ac_dim'],
               hparams['ob_dim'],
               hparams['n_layers_critic'],
               hparams['size_hidden_critic'],
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        next_ac_na = self.actor_target(next_ob_no)
        # TODO compute the Q-values from the target network
        ## Hint: you will need to use the target policy
        qa_tp1_values = self.q_net_target(next_ob_no, next_ac_na).squeeze()
