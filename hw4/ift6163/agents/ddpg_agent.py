...
class DDPGAgent(object):
    ...
    def step_env(self):
        ...
        action = self.actor.get_action(obs) + np.random.normal(0,1)*0.1
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            for critic_update in range(self.agent_params["num_critic_updates_per_agent_update"]):
                ...
                log['Critic'] = self.q_fun.update(ob_no, ac_na, next_ob_no, re_n, terminal_n)

            # TODO fill in the call to the update function using the appropriate tensors
            ## Hint the actor will need a copy of the q_net to maximize the Q-function
            log['Actor'] = self.actor.update(ob_no, copy.copy(self.q_fun))
