...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        if itr == 0:
            ...
            path = hydra.utils.to_absolute_path(load_initial_expertdata)
            loaded_paths = pickle.load(open(path, 'rb'))
            return loaded_paths, 0, None
        
        ep_len = self.params['env']['max_episode_length']
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            good_label = expert_policy.get_action(paths[i]['observation'])
            paths[i]['action'] = good_label
