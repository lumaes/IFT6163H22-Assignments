...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        ...
        shuffled_indexes = np.random.permutation(np.arange(self.obs.shape[0]))
        batch_indexes = shuffled_indexes[:batch_size]

        return (self.obs[batch_indexes], 
                self.acs[batch_indexes], 
                self.rews[batch_indexes], 
                self.next_obs[batch_indexes], 
                self.terminals[batch_indexes])
